...
* `// normalize-stderr-test "targetSpecificMsg" -> ""` Replaces any substrings in stderr with another to normalise output
  ...
  * remove debug info:
    * `// normalize-stderr-test "\n\W*OpLine .*" -> ""` remove all line numbers
    * `// normalize-stderr-test "\n\W*OpSource .*" -> ""` remove all source code
    * `// normalize-stderr-test "\n\W*%\d+ = OpString .*" -> ""` remove all src file paths
  * when disassembling globals and testing on both `vulkan` and `spv` targets:
    * `// normalize-stderr-test "\n\W*OpCapability VulkanMemoryModel" -> ""` remove the vulkan memory model *capability*, only used by `vulkan` targets
    * `// normalize-stderr-test "\n\W*OpExtension .SPV_KHR_vulkan_memory_model." -> ""` remove the vulkan memory model *extension*, only used by `vulkan` targets
    ...
    * `// normalize-stderr-test "\n\W*; .*" -> ""` remove comments on spirv version by rspirv
